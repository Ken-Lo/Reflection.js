...
Range.prototype.setValue = function (value,nocall) {
	...
	if (this._value != value) {
		if (value + this._extent > this._maximum)
			...
			this._value = value;
		if (!this._isChanging && typeof this.onchange == "function" && !nocall) {
			 this.onchange();
		 }
	}
};
...
Range.prototype.setMaximum = function (maximum) {
	if (this._maximum != maximum) {
		...
		if (maximum < this._value)
			this.setValue(maximum - this._extent, 1);
		if (maximum < this._minimum) {
			this._extent = 0;
			this.setMinimum(maximum);
			this.setValue(this._maximum, 1);
		}		
